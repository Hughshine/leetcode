...
二分是根据“k”做二分的（每一次，一定会约掉k/2个数，如果数组不够大，下一次就不需要再考虑这个数组。注意！！），搜索空间就是两个数组的各自前k个数。这种递归都怎么方便怎么来，注意递归终点的情景。
...
public:
    int inline findKth(vector<int>& nums1, vector<int>& nums2, int s1, int e1, int s2, int e2, int k){
        ...
        int mid1 = min(e1, s1+k/2 - 1), mid2 = min(e2, s2+k/2 - 1);  // 这个坐标的计算，很重要。要知道，要舍去哪部分。
        // cout << mid1 << " " << mid2 << endl;
