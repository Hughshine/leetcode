...
AVL实际就是，先按BST做，然后balance被改变的节点。（一般就是递归实现）

lrotate和rrotate实现的不够好看，可读性很差。。

额外注意，在使用引用时，如果还调用了其他函数，要考虑会不会有影响。本问题中是递归+自身重新调用，debug了好久好久。remove参数使用了指针引用，但在孩子有两个的时候，会改成删后继节点，删除后继节点可能导致全部的树的调整，进而导致指针引用指向错误。
